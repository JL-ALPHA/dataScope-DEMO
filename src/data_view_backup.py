...
class DataViewWidget:
    ...
    def _perform_real_time_search(self) -> None:
        ...
        try:
            ...
            # Perform optimized search with result limiting
            search_result = search_dataframe_optimized(
                search_df, 
                self.search_term, 
                column, 
            ...
            if results:
                ...
                displayed_count = len(limited_results)
                
                # Update UI feedback with search options info and performance data
                match_text = f"{displayed_count}"
                if results_count > self.max_search_results:
                    match_text += f" of {results_count}+"
                match_text += f" match{'es' if results_count != 1 else ''}"
                
                options_text = []
                if case_sensitive:
                    options_text.append("case-sensitive")
                if whole_word:
                    options_text.append("whole words")
                if self.search_visible_only:
                    options_text.append("visible only")
                
                if self.match_label:
                    self.match_label.value = match_text
                    self.match_label.color = ft.Colors.GREEN_600
                    self.match_label.update()
                
                if self.search_status:
                    status_msg = f"✓ Found {match_text}"
                    if results_count > self.max_search_results:
                        status_msg += f" (limited to {self.max_search_results})"
                    if options_text:
                        status_msg += f" ({', '.join(options_text)})"
                    status_msg += f" in {search_duration:.2f}s"
                    
                    self.search_status.value = status_msg
                    self.search_status.color = ft.Colors.GREEN_600
                    self.search_status.update()
                
                # Store highlight terms for visual feedback
                self.highlight_term = self.search_term
                self.highlight_column = column
                
                # Update dynamic status for search results
                self.update_status_message('search', 
                                         matches=results_count, 
                                         term=self.search_term)
                
                self._refresh_display()
                print(f"[DataView] Optimized search found {displayed_count} of {results_count} matches for '{self.search_term}' in {search_duration:.3f}s (case: {case_sensitive}, whole: {whole_word}, visible_only: {self.search_visible_only})")
            else:
                # No results found
                if self.match_label:
                    self.match_label.value = "No matches"
                    self.match_label.color = ft.Colors.RED_600
                    self.match_label.update()
                
                if self.search_status:
                    status_msg = f"⚠ No results found in {search_duration:.2f}s"
                    if self.search_visible_only:
                        status_msg += " (visible data)"
                    self.search_status.value = status_msg
                    self.search_status.color = ft.Colors.RED_600
                    self.search_status.update()
